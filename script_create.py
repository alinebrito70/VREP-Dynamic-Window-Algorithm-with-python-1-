...
def get_ob(x):
    _c, _h, status, distances, _s = ri.getDistanceCollection()
    ob = [[-1,-1]]
    if len(status) != 0: # nenhum objeto encontrado
     for i in range(16):
        if status[2*i]!=0: # verifica se aquele sensor identificou um valor válido
            distance = (distances[6*i + 0]**2 + # distância euclidiana 
                        distances[6*i + 1]**2 + #do sensor ao ponto identificado
                        distances[6*i + 2]**2)**(1/2)
            orientation = sensor_ori[i] + x[2] # obter o angulo real do sensor
            position = [x[0] + sensor_pos[i][0] + distance*math.cos(orientation),
                        x[1] + sensor_pos[i][1] + distance*math.sin(orientation)]
                        # estimar a posição real do objeto
                        # somar posição real do robo,
                        # posição relativa do sensor com o robo
                        # distancia dividida em coordenadas cartesianas
            ob.append(position)#adicionar obstaculo identificado no vetor de obstaculos
...
def init():
    ...
    param['motorLeft'] = ri.getobject("MOTOR_ESQUERDO")
    param['motorRight'] = ri.getobject("MOTOR_DIREITO")
    ...
    config.dt = 0.2  # [s] Time tick for motion prediction
...
def finnaly():
    print("Done")
    if dw.show_animation:
        gs = gridspec.GridSpec(2, 2) #gridspec 3x3
        x = param['x']
        plt.subplot(gs[1,1])
        ...
        plt.subplot(gs[0,0])
        plt.plot(wr_plot)
        plt.title("Velocidade angular R")

        plt.subplot(gs[0,1])
        plt.plot(wl_plot)
        plt.title("Velocidade angular L")
        plt.pause(0.1)

    plt.show()
    

ri.start(init,loop,finnaly,60)